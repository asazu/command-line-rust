...
fn dies_not_enough_args() -> Result<()> {
    dies(
        ...
        <--bytes <LIST>|--characters <LIST>|--fields <LIST>>",
    )
}
...
fn dies_bad_digit_field() -> Result<()> {
    ...
    dies(
        ...
        &format!(r#"invalid field value '{}'"#, &bad),
    )
}
...
fn dies_empty_delimiter() -> Result<()> {
    dies(
        ...
        r#"invalid value '' for '--delimiter <DELIM>':"#,
    )
}
...
fn tsv_f1_stdin() -> Result<()> {
    run_stdin(TSV, &["-f", "1"], "tests/expected/movies1.tsv.f1.out")
}

// --------------------------------------------------
#[test]
fn tsv_f1_stdin2() -> Result<()> {
    run_stdin(TSV, &["-f", "1", "-"], "tests/expected/movies1.tsv.f1.out")
}
